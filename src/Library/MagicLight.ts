...
class MagicLight {
    ...
    return new Promise<BulbInfo>((res, rej) => {
      ...
      exec(`flux_led -i ${this.bulbIP}`, (error, stdout, stderr) => {
        if (error || stderr) rej(error ? error : stderr);
          ...
          let temp = stdout
            .substr(stdout.indexOf(']') + 1, stdout.length)
            .split(' ')
          ...
          const obj: BulbInfo = {
            ...
            rgb: {
              r:
                temp[2] === 'White:'
                  ? 0
                  : parseInt(temp[2].replace(/[(|)]/g, '')),
              g: temp[2] === 'White:' ? 0 : parseInt(temp[3]),
              b:
                temp[2] === 'White:'
                  ? 0
                  : parseInt(temp[4].replace(/[(|)]/g, '')),
            },
            warmWhite:
              temp[2] === 'White:' ? parseInt(temp[3].replace('%', '')) : 0,
  ...
  public setRGB(rgb: { r: number; g: number; b: number }): Promise<Boolean> {
    return new Promise<Boolean>((res, rej) => {
      exec(
        `flux_led ${this.bulbIP} -c ${rgb.r},${rgb.g},${rgb.b}`,
        (error, stdout, stderr) => {
          if (error || stderr) rej(error ? error : stderr);
          res(true);
        }
  ...
  public setCold(val: number): Promise<Boolean> {
    return new Promise<Boolean>((res, rej) => {
      exec(
        `flux_led ${this.bulbIP} --coldwhite ${val}`,
        (error, stdout, stderr) => {
          if (error || stderr) rej(error ? error : stderr);
          res(true);
        }
