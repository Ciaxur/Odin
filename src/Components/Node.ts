...
export interface NodeEventExec {
 ...
 */
export function getEventExec(
  eventExec: NodeEventExec,
  lightAddr: string,
  event: NodeEvent,
  ...
  return () => {
    ...
    const res = handleLightAction(
      {
        ...
        actionValue: eventExec.value as
          | string
          | number
          | boolean
          | NodeEventExec,
    ...
    if (res)
      eventCollection[lightAddr] = eventCollection[lightAddr].filter(
        (e) => e != event
      );
  };
}
